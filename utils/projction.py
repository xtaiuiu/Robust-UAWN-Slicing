...
from algorithms.rounding.rounding_algorithms import rounding_by_opt_condition
...
def proj_generalized_simplex_lb(a, y, r, l):
    ...
    # assert abs(np.dot(a, z) - r) < 1e-8, f"assert error in projection: np.dot(a, z) = {np.dot(a, z)}, r = {r}"
    return z + l


def proj_nonnegative_half_space_lb(a, y, r, l):
    """
    Project y onto the non-negative half space by KKT conditions
    a^T x <= r, x >= l
    :param a: a numpy array, must be a positive array
    :param y: a numpy array
    :param r: float, must be positive
    :param l: a numpy array represents the lower bound, must be positive
    :return: a numpy array
    """
    z = proj_nonnegative_half_space(a, y-l, r - a@l)
    return z + l

def proj_nonnegative_half_space(a, y, r):
    """
    Project y onto the non-negative half space by KKT conditions
    a^T x <= r, x >= 0
    :param a: a numpy array, must be a positive array
    ...
    :return: a numpy array
    """
    y_plus = np.maximum(y, np.zeros_like(y))  # y_plus = y^+
    if a@y_plus <= r:
        return y_plus
    else:
        return proj_generalized_simplex(a, y_plus, r)


def proj_nonnegative_half_space_QP(a, y, r, l):
    """
    Project y onto the non-negative half space by QP using cvxpy
    a^T x <= r, x >= l
    """
    if a@y <= r and np.all(y >= l):
        return y
    else:
        x = cp.Variable(len(y))
        objective = cp.Minimize(cp.norm(x - y))
