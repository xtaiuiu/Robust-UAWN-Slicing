...
def run_save(n_repeats=100):
    ...
    df_bandwidth_UE_avg = pd.DataFrame(np.zeros((len(bandwidths), len(pd_columns))), columns=pd_columns)

    for repeat in range(n_repeats):
        print(f" ###################################### repeat = {repeat} #######################################")
        df_rate = pd.DataFrame(np.zeros((len(bandwidths), len(pd_columns))), columns=pd_columns)
        for i in range(len(bandwidths)):
            ...
            f_sqp, x_sqp, p_sqp, _ = Lag_SQP_alg(prob, prob.x_u, prob.p_u)
...
def load_plot():
    ...
    ax = df_DAL_iter_avg.plot(legend=True, lw=2, xlabel=r'Total bandwidth $B_{tot}$ (MHz)',
                              ylabel='Avg. data rate (Mbps)',
