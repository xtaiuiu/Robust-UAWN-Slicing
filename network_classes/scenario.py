...
class Scenario:
    ...
    def set_UE_Delta_g(self, epsilon, delta):
        """
        In our paper, we assume \tilde{g} = bar{g} + \Delta{g}, where \Delta{g} ~ N(0, \delta)
        is a normal distributed random variable.
        Set \tilde{g} = bar{g} + \Delta{g}
        :return: None
        """
        for s in self.slices:
            for u in s.UEs:
                #print(f" old gain = {u.tilde_g/(self.uav.theta**2 * self.pn.sigma * (self.uav.h**2 + u.loc_x**2 + u.loc_y**2)**(self.pn.alpha/2))}")
                u.tilde_g += np.sqrt(2) * erfcinv(2 * (1 - epsilon)) * delta
                #print(f" new gain = {u.tilde_g/(self.uav.theta**2 * self.pn.sigma * (self.uav.h**2 + u.loc_x**2 + u.loc_y**2)**(self.pn.alpha/2))}")

    def reset_UE_Delta_g(self):
        for s in self.slices:
            for u in s.UEs:
                u.tilde_g = 2.6
    ...
    def get_UE_rate(self):
        ...
        for s in self.slices:
            for u in s.UEs:
                rate = u.x * s.b_width * np.log1p(pn.g_0 * u.tilde_g * u.p / (
