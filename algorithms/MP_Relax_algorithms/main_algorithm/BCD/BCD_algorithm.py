...
def bcd_al(prob, lam, rho, x_init, p_init, subx_optimizer=optimize_x_cvx,
    ...
    while abs((f - f_old)/f_old) > eps and abs(f - f_old) > eps and n < n_max:
        ...
        if x_first:
            ...
            if abs(f - f_old) < eps or abs((f - f_old) / f) < eps:
                break
            print(f"n = {n}, optimize x: lam = {lam: .4f}, rho = {rho: .8f}, f_old = {f_old: .8f}, f = {f: .8f}, time = {time.perf_counter() - t1: .8f}")
            t1 = time.perf_counter()
            f_old = f
            ...
            if abs(f - f_old) < eps or abs((f - f_old) / f) < eps:
                break
            print(f"n = {n}, optimize p: lam = {lam :.4f}, rho = {rho: .8f}, f_old = {f_old:.8f}, f = {f:.8f}, time = {time.perf_counter() - t1: .8f}")
        n += 1
